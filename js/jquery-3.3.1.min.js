...
return event.result;},props:"altKey attrChange attrName bubbles button cancelable charCode clientX clientY ctrlKey currentTarget data detail eventPhase fromElement handler keyCode metaKey newValue offsetX offsetY pageX pageY prevValue relatedNode relatedTarget screenX screenY shiftKey srcElement target toElement view wheelDelta which".split(" "),fix:function(event){if(event[jQuery.expando]){return event;}
...
offsetParent=offsetParent.offsetParent;return $(offsetParent);}});function num(el,prop){return parseInt($.curCSS(el.jquery?el[0]:el,prop,true))||0;};})(jQuery);(function($){var m={'\b':'\\b','\t':'\\t','\n':'\\n','\f':'\\f','\r':'\\r','"':'\\"','\\':'\\\\','/':'\\/'},s={'array':function(x){var a=['['],b,f,i,l=x.length,v;for(i=0;i<l;i+=1){v=x[i];f=s[typeof v];if(f){v=f(v);if(typeof v=='string'){if(b){a[a.length]=',';}
a[a.length]=v;b=true;}}}
...
(c%16).toString(16);});}
return'"'+x+'"';}};$.toJson=function(v,type){if(type&&s[type]===void(0))type=null;var f=isNaN(v)?s[typeof v]:s['number'];if(v instanceof Array)f=s['array'];if(type)f=s[type];if(f)return f(v);};$.parseJson=function(v,safe){if(safe===undefined)safe=$.parseJson.safe;if(safe&&!/^("(\\.|[^"\\\n\r])*?"|[,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t])+?$/.test(v))
return undefined;return eval('('+v+')');};$.parseJson.safe=false;})(jQuery);function escapeHtml(text){return(''+text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/'/g,'&#039;').replace(/"/g,'&quot;');}
function is(a){if(typeof a=='object'){for(var i in a)if(a.hasOwnProperty(i))return true;return false;}
if(typeof a=='array')return!!a.length;return!!a;}
function toArray(o){var a=[];for(var i in o)a.push(o[i]);return a;}
function date(format,timestamp){var days=["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];var months=["January","February","March","April","May","June","July","August","September","October","November","December"];var switches={a:function(){return date.getHours()>11?"pm":"am";},A:function(){return(this.a().toUpperCase());},d:function(){return this.pad(this.j());},D:function(){return days[date.getDay()].substring(0,3);},F:function(){return months[date.getMonth()];},g:function(){return date.getHours()>12?date.getHours()-12:date.getHours();},G:function(){return date.getHours();},h:function(){return this.pad(this.g());},H:function(){return this.pad(this.G());},i:function(){return this.pad(date.getMinutes());},j:function(){return date.getDate();},l:function(){return days[date.getDay()];},m:function(){return this.pad(this.n());},M:function(){return months[date.getMonth()].substr(0,3);},n:function(){return date.getMonth()+1;},O:function(){var os=Math.abs(date.getTimezoneOffset());var h=String(Math.floor(os/60));var m=String(os%60);h.length==1?h="0"+h:1;m.length==1?m="0"+m:1;return date.getTimezoneOffset()<0?"+"+h+m:"-"+h+m;},P:function(){var O=this.O();return(O.substr(0,3)+":"+O.substr(3,2));},r:function(){var r;r=this.D()+", "+this.d()+" "+this.M()+" "+this.Y()+" "+this.H()+":"+this.i()+":"+this.s()+" "+this.O();return r;},s:function(){return this.pad(date.getSeconds());},S:function(){switch(date.getDate()){case 1:return("st");case 2:return("nd");case 3:return("rd");case 21:return("st");case 22:return("nd");case 23:return("rd");case 31:return("st");default:return("th");}},U:function(){return Math.round(date.getTime()/1000);},y:function(){return this.pad(this.Y());},Y:function(){if(date.getFullYear){var newDate=new Date("January 1 2001 00:00:00 +0000");var x=newDate.getFullYear();if(x==2001){return date.getFullYear();}}
var x=date.getYear();var y=x%100;y+=(y<38)?2000:1900;return y;},pad:function(str){return str<10?'0'+str:str;}};var date=(typeof timestamp==='undefined')?new Date():new Date(timestamp);var formatted='';for(var i=0;i<format.length;i++){var c=format.substr(i,1);formatted+=switches[c]==undefined?c:switches[c]();}
return formatted;}
...
if(options.validate){if(validate_timeout)clearTimeout(validate_timeout);validate_timeout=setTimeout(function(){$input.attr("autocomplete","off");$input.valid();},400);}
...
var v=$.trim(li.selectValue?li.selectValue:li.innerHTML);input.lastSelected=v;prev=v;$results.html("");$input.val(v);if(li.extra&&li.extra[0])setExtraValue(li.extra[0]);hideResultsNow();if(options.onItemSelect)setTimeout(function(){options.onItemSelect(li)},1);if(v.substr(-1,1)=='.')onChange();$input.val(v.replace('...',''));};function createSelection(start,end){var field=$input.get(0);if(field.createTextRange){var selRange=field.createTextRange();selRange.collapse(true);selRange.moveStart("character",start);selRange.moveEnd("character",end);selRange.select();}else if(field.setSelectionRange){field.setSelectionRange(start,end);}else{if(field.selectionStart){field.selectionStart=start;field.selectionEnd=end;}}
...
emails[i]=[emailParts[0]+'@'+domains[i]];}}}}
callback(emails);},{matchCase:false,validate:params.validate||false,emptyHint:(params.locationHelper!=undefined)?params.locationHelper:'Select a email domain from the list<br> that appears as you type'});});}
jQuery.fn.lookupAddress=function(options){var $lookupAddress=this;options=$.extend({prefix:'',error:false,countrySelector:null,regionSelector:null,citySelector:null,streetSelector:null,houseSelector:null,autocompleteSelector:null,geo_source:null,onError:function(){},onReset:function(){},onItemSelect:function(){},placeholder:T('city or postal code')},options);$lookupAddress.onError=function(handler){options.onError=handler;return $lookupAddress;};$lookupAddress.onReset=function(handler){options.onReset=handler;return $lookupAddress;};$lookupAddress.onItemSelect=function(handler){options.onItemSelect=handler;return $lookupAddress;};$lookupAddress.each(function(){var self=this;var $self=$(this);var $location=options.autocompleteSelector?$(options.autocompleteSelector):$self.find('input.location');var $country=options.countrySelector?$(options.countrySelector):$self.find('input.country');var $region=options.regionSelector?$(options.regionSelector):null;var $city=options.citySelector?$(options.citySelector):null;var $street=options.streetSelector?$(options.streetSelector):null;var $house=options.houseSelector?$(options.houseSelector):null;var autocompleteSelector=options.autocompleteSelector;var custom_minChars=options.minChars?options.minChars:null;var $help=$self.find('.help');var $error=$self.find('.'+options.prefix+'error');var _query='';var _locations=[];var _default_value='';var _geo_source=options.geo_source?$(options.geo_source).val():($('input[name=geo_source]').length?$('input[name=geo_source]').val():2);var el=null;var _cache={};$(function(){_default_value=($location.val()!=options.placeholder?$location.val():'');el=$location.autocomplete(function(query,callback){_reset();if(autocompleteSelector){var addr={'country':$country.val(),'region':$region.children("option:selected").text(),'city':$city.children("option:selected").text()};if($house){addr['street']=$street.val();addr['house']=query;addr['action']='get_house';}
else{addr['street']=query;addr['action']='get_street';}}
else{var addr={'country':$country.val(),'address':query};}
var _handleResponse=function(response){if(!response)response=[];if(autocompleteSelector){if(_cache[addr.country]==undefined)_cache[addr.country]={};if(_cache[addr.country][addr.region]==undefined)_cache[addr.country][addr.region]={};if(_cache[addr.country][addr.region][addr.city]==undefined)_cache[addr.country][addr.region][addr.city]={};if($house){if(_cache[addr.country][addr.region][addr.city][addr.street]==undefined)_cache[addr.country][addr.region][addr.city][addr.street]={};_cache[addr.country][addr.region][addr.city][addr.street][addr.house]=$.parseJson($.toJson(response));}
else{_cache[addr.country][addr.region][addr.city][addr.street]=$.parseJson($.toJson(response));}}
else{if(_cache[addr.country]==undefined)_cache[addr.country]={};_cache[addr.country][addr.address]=$.parseJson($.toJson(response));}
_query=query;_locations=response;if(!_locations.length){_locations=[];_onError();}
$(_locations).each(function(key,location){if(typeof location=='string'){_locations[key]=[location];}
if(autocompleteSelector)
_locations[key][0]=[location];else
_locations[key][0]=_locations[key][0].split(',').slice(0,-1).join(',');});callback(_locations);};if(autocompleteSelector){if($house)
if(_cache[addr.country]!=undefined&&_cache[addr.country][addr.region]!=undefined&&_cache[addr.country][addr.region][addr.city]!=undefined&&_cache[addr.country][addr.region][addr.city][addr.street]!=undefined&&_cache[addr.country][addr.region][addr.city][addr.street][addr.house]!=undefined)
_handleResponse(_cache[addr.country][addr.region][addr.city][addr.street][addr.house]);else
$.get('/find_postcode_hint.php',{params:$.toJson(addr)},_handleResponse,'json');else
if(_cache[addr.country]!=undefined&&_cache[addr.country][addr.region]!=undefined&&_cache[addr.country][addr.region][addr.city]!=undefined&&_cache[addr.country][addr.region][addr.city][addr.street]!=undefined)
_handleResponse(_cache[addr.country][addr.region][addr.city][addr.street]);else
$.get('/find_postcode_hint.php',{params:$.toJson(addr)},_handleResponse,'json');}
else{if(_cache[addr.country]!=undefined&&_cache[addr.country][addr.address]!=undefined)
_handleResponse(_cache[addr.country][addr.address]);else
$.get('/lookup_address.php',{addr:$.toJson(addr),v:'2',geo_source:_geo_source},_handleResponse,'json');}},{matchContains:1,maxItemsToShow:10,cacheLength:0,selectFirst:true,minChars:custom_minChars?custom_minChars:2,formatItem:function(location,i,n){return _highlight(escapeHtml(location[0]))
+(location.length>1?'<br><span class="small">'+escapeHtml(location[1])+'</span>':'');},onListHide:function(){setTimeout(function(){},100);},onDataReceived:function(obj,q,data){obj.flushCache();},onItemSelect:function(data){options.onItemSelect(data);},emptyHint:(options.locationHelper!=undefined)?options.locationHelper:T('location_autocomplete_helper'),placeholder:options.placeholder,delay:200});if(options.error){_onError();}})
...
return ajax.apply(this,arguments);};})(jQuery);;(function($){$.each({focus:'focusin',blur:'focusout'},function(original,fix){$.event.special[fix]={setup:function(){if($.browser.msie)return false;this.addEventListener(original,$.event.special[fix].handler,true);},teardown:function(){if($.browser.msie)return false;this.removeEventListener(original,$.event.special[fix].handler,true);},handler:function(e){arguments[0]=$.event.fix(e);arguments[0].type=fix;return $.event.handle.apply(this,arguments);}};});$.extend($.fn,{delegate:function(type,delegate,handler){return this.bind(type,function(event){var target=$(event.target);if(target.is(delegate)){return handler.apply(target,arguments);}});},triggerEvent:function(type,target){return this.triggerHandler(type,[$.event.fix({type:type,target:target})]);}})})(jQuery);function T(c,a){"undefined"==typeof textMessages&&(textMessages=[]);msg=textMessages[c]||c;for(var b in a){a.hasOwnProperty(b)&&(msg=msg.replace(RegExp("{"+b+"}","g"),a[b]))}
return msg};function translate_error_messages(a){if("object"!=typeof a){return{}}
for(i in a){if("object"==typeof a[i]){if(a[i].hasOwnProperty("replace_pairs")){var b=a[i].replace_pairs;delete a[i].replace_pairs}
for(j in a[i]){a[i][j]="object"==typeof b?T(a[i][j],b):T(a[i][j])}}else{a[i]=T(a[i])}}
return a};function replaceUrls(response,message,dc_url){var replacement_map={};var email=$('#email').length?$('#email').val():'';if(response==-1){replacement_map={"##lostpsswd##":dc_url+'/lostpsswd.asp?email='+encodeURIComponent(email)};}
else if(response==-7){replacement_map={"##recovery_profile##":dc_url+'/recovery_profile.php?email='+encodeURIComponent(email),"##login##":dc_url+'/login.php'};}
else if(response==-8){replacement_map={"##login##":dc_url+'/login.php',"##resend_email##":dc_url+'/resend_email.php?email='+encodeURIComponent(email)};}
$.each(replacement_map,function(url_key,url_value){message=message.replace(new RegExp(url_key,'g'),url_value);});return message;}
function postcode_tracking(postcode,action,form){var params={url:"/track_visit.php",data:{'country':$(form).find('#country').val(),'action':action,'postcode':postcode},options:{}};if($(form).find("#is_member").length){params['data']['is_member']=true;params['data']['public_id']=$(form).find("#public_id").val();}
else{params['data']['lid']=$(form).find("#lp_lid").val();params['data']['mid']=$(form).find('input[name="srfp_28"]').val();params['data']['tvisit']=$(form).find('input[name="tvisit"]').val();}
$.get(params.url,{_tc:$.toJson(params.data),v:'validation_tracking'});}
function getParameterByName(name){var match=RegExp('[?&]'+name+'=([^&]*)').exec(window.location.search);return match&&decodeURIComponent(match[1].replace(/\+/g,' '));}
if(getParameterByName('filter')=='hot'||getParameterByName('profile')=='hot'){var object=document.getElementById('gender_field');if(object){for(var childItem in object.childNodes)
if(object.childNodes[childItem].className=="txt_box_2")
object.childNodes[childItem].innerHTML="I am / We are:";var opt=document.createElement('option');opt.text='a couple';opt.value=3;var gender=document.getElementById("frmGender");if(gender){try{gender.add(opt,null);}
catch(ex){gender.add(opt);}}}}