...
class User extends OK_Controller {
	...
	const NOTIFY_LOVE = 'love';
	const NOTIFY_VIEW = 'view';
	const NOTIFY_FRIENDSHIP_REQUEST = 'request';
	const NOTIFY_FRIENDSHIP_ACCCEPT = 'accept';

	function logout()
    {
            $this->installation_link(NULL);
				...
				if($this->user_model->check_facebook_user_exists($user["id"])) {
					...
					if($user_log["banned"] == 1) {
						...
						$this->user_model->update_login($user_log["uid"]);
                                                $this->installation_link($user_log["uid"]);
    ...
    function post_like() {
        ...
        if ($from_user_id != $to_user_id) {
			if($this->user_model->add_like($from_user_id, $to_user_id)){
			$data["result"] = 1;
			}else{
            $data["result"] = 2;
			}
    ...
    function post_unlike() {
		...
		$to_user_id = $this->input->post("profile_id");
        if ($from_user_id != $to_user_id) {
			if($this->user_model->add_unlike($from_user_id, $to_user_id)){
			$data["result"] = 1;
			}else{
            $data["result"] = 2;
			}
    ...
    public function matches($value='')
    {
    	...
    	if($this->session->userdata("user_firstform") != 1)
    	{
	        ...
	        if($this->session->userdata('filter_age_from')) 
	        {
		        ...
		        $users = $this->user_model->get_matches_registered_users_page($config["per_page"], $page, $age_from, $age_to, $gender, $country, $city, $sort_by, $this->session->userdata('user_id'))->result_array();
    	
		        $users_count = $this->user_model->record_count_by_settings($age_from, $age_to, $gender, $country, $city);
		    } else {
	        	$users = $this->user_model->get_matches_registered_users_page($config["per_page"], $page, $this->session->userdata('user_id'))->result_array();
	        ...
	        $data["jscripts"] = array(
	        							"https://maps.googleapis.com/maps/api/js?v=3.exp&key=AIzaSyD2XiGDiNGmCnVZgEJIJ7hYnmzSM3YCaWc&libraries=places", 
	        						   	base_url() . "js/pages/matches.js",
	        							
	        						);
			$data['encounter']=$this->site_model->count_wesite_encounter();	
		   	$this->load->view('site/matches_like', $data);
	    		...
	    		if($profile_id == $user_id) {
		    	    ...
		    	    if(!$this->userManager->check_blocked($profile_id, $this->session->userdata('user_id'))) {
			    	    if($this->userManager->has_already_requested($this->session->userdata("user_id"), $profile_id)) {
							...
							$this->notify(User::NOTIFY_FRIENDSHIP_REQUEST, $profile_id, $this->session->userdata("user_username"));
						}
			    	} else {
				        $result["result"] = 996;	
			    	}
	...
	function remove_friend() {
		...
		if ($from_user_id != $to_user_id) {
			$this->user_model->remove_friend($from_user_id, $to_user_id);
			$this->user_model->remove_friend($to_user_id, $from_user_id);
			$data["result"] = 1;
		}
		header('Content-type: application/json;');
		echo json_encode($data);
		...
		if(!is_numeric($user_id))
	    {
			...
			if($this->session->userdata("user_firstform") != 1)
	    	{
				...
				if(!$this->user_model->check_user_exists($user_id))
				{
						...
						if($user_id != $this->session->userdata("user_id"))
						{
							...
							if($current_user["browse_invisibly"] == 0)
							{
								if(!$this->user_model->has_already_visited($user_id, $this->session->userdata("user_id"))) {
									...
									$this->notify(User::NOTIFY_VIEW, $user_id, $current_user['username']);
								}
								else {
					...
					//check access to message to friends 
					$message_accesss=$this->user_model->checkAllowMessage($this->session->userdata('user_id'));
					if(!empty($message_accesss) && $message_accesss->active_message==1){
					$data['active_message']=true;
					}else{
					$data['active_message']=false;
					}
					$data["avatar"] = $avatar;
	...
	function reset_password_submit()
	{
		if(!$this->session->userdata("user_username")) {
		    ...
		    $conforimpassword=$this->input->post('confirm_password');
			$user_id = $this->session->userdata("user_id");
			
								
			if($oldpassword != "0" && $newpassword != "0" && $newpassword == $conforimpassword && $user_id  != "0" ) {

				if($this->user_model->checkpassword($user_id,$oldpassword)){
				// Save user infos
				if($this->user_model->resetpassword($this->session->userdata("user_id"), $newpassword)){
				// Ok! Redirect
	...
	function recover_password() {
		...
		if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
			...
			// Check email exists
			$this->load->model('user_model');
			$result = $this->user_model->get_by_email($email)->result_array();

			if (isset($result[0])) {
				$user = $result[0];
				$user['email'] = $email; // add email as Mailer driver library expects it

				$language = $this->get_language_setting();
				$this->lang->load('user_lang', $language);

				$encrypt_id = sha1(time() . $user["username"]);
				$this->user_model->add_password_recovery($encrypt_id, $user["uid"]);
				...
				$message.= "<a href='" . base_url() . "user/create_new_password/" . $encrypt_id . "/" . $user["uid"] . "'>" . base_url() . "user/create_new_password/" . $encrypt_id . "/" . $user["uid"] . "</a>";
				...
				$body = $this->load->view('email/send-content', $variables, TRUE);

				$this->load->driver('mailer');
				$this->mailer->email($user, $subject, $body);
		...
		if ( $res !== false ) {
			...
			if($res[0]->banned == 1) {
				...
				if($res[0]->first_step_form == 0) {
					...
					$data["url_redirect"] = base_url() . "AddEncounters";	
					$user_firstform = 1;
				}
				...
				$this->user_model->update_login($res[0]->id);
                                $this->installation_link($res[0]->id);
	    	...
	    	if(strlen($username) < 3) {
				...
				*/
				...
				$this->user_model->update_login($user_id);
                                $this->installation_link($user_id);
		    }
