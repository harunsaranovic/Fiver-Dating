<?php
class pm extends OK_Controller {
		    	...
		    	if($form_message != "")
		    	{
					...
					if($conv != NULL) {
						if(!$this->userManager->check_blocked($user_id, $this->session->userdata('user_id'))) {
							...
							$notified_today = $this->pmManager->check_daily_notif($receiver["uid"]);

							if (!$notified_today) {
								$this->pmManager->add_daily_notif($receiver["uid"]);
							}

							$this->notify($receiver, $this->session->userdata("user_username"), !$notified_today);
    	...
    	if (!$this->session->userdata("user_id")) {								
			...
			if(sizeof($user) == 0) {
				...
				if($current_conv != null) {
					...
					//echo '<pre/>',print_r($message_accesss); die();
			    		...
			    		if($this->session->userdata("user_firstform") == 0) {
					    	...
					    	if($receiver != NULL)
							{
								if(!$this->userManager->check_blocked($user_id, $this->session->userdata('user_id'))) {
									...
									$notified_today = $this->pmManager->check_daily_notif($receiver[0]["uid"]);

									if (!$notified_today) {
										$this->pmManager->add_daily_notif($receiver[0]["uid"]);
									}

									$this->notify($receiver[0], $this->session->userdata("user_username"), !$notified_today);
	...
	/**
	 * Prepares and sends PM notification.
	 * 
	 * @param array $receiver
	 * @param string $sender_username
	 * @param boolean $send_email
	 */
	private function notify($receiver, $sender_username, $send_email = TRUE) {
		$language = $this->get_language_setting($receiver['uid']);
		$this->lang->load('notify_lang', $language);

		$this->load->driver('notification');
		$this->notification->push = sprintf($this->lang->line('notify_pm_push'), $sender_username);

		if ($send_email) {
			$this->notification->subject = $this->lang->line('notify_pm_subject');
			$this->notification->message = $this->lang->line('notify_pm_message');
		}

		$this->notification->send($receiver['uid']);
	}
