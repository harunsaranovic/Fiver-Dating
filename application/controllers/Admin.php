...
class Admin extends CI_Controller {
    ...
    function edit_user($user_id = 0) 
    {
		...
		if($this->session->userdata("user_rank") < 1) {
	    	...
	    	if(sizeof($user) == 0) {
				...
				$data["photos"] = $this->user_model->get_photos($user_id, 50)->result_array();
	...
	/**
	 * Updates photo and thumbnail. Its invoked from Pixie onSave callback.
	 * 
	 * Requires $_REQUEST['id'] which is photo id and base64 encoded image data in $_REQUEST['contents'].
	 * 
	 * Echos JSON {success: true} when successful.
	 */
	function update_photo() {
		if ($this->session->userdata("user_rank") < 1) {
			...
			$result = $this->photo_model->get($id)->result_array();

			if (sizeof($result) == 1) {
				$photo = $result[0];
				$photoPath = "." . $photo["url"];
				$thumbPath = "." . $photo["thumb_url"];

				if (file_put_contents($photoPath, file_get_contents($contents))) {
					image_compress($photoPath, $thumbPath, 100, 800);
    ...
    function manage_users_access()
    {
    	...
    	if($this->session->userdata("user_rank") < 1) {
    		...
    		$data["jscripts"] = array(base_url()."js/pages/admin_manage_users.js");
	    	$this->load->view('admin/manage_users_access', $data);
	   	} 
    }
  ...
  function customers() 
    {
    	...
    	$data["title"] = "Admin - OkDate Customers";
	    ...
	    $data = array();
	   	...
	   	if(DEMO_MODE == 1) {
	    	...
	    	if($this->session->userdata("user_rank") < 1) {
				...
				$spot_light_price = $this->input->post('spot_light_price');
				$active_message_price = $this->input->post('active_message_price');
				...
				if($enable_payment == 1) {
					...
					$this->site_model->update_settings(array(
						...
						"spot_light_price"			=> $spot_light_price,
						"active_message_price"	    => $active_message_price,
		...
		if(DEMO_MODE == 1) {
		    ...
		    if($this->session->userdata("user_rank") < 1) {
				...
				if(strlen($web_title) < 3) {
					...
					$data["error"] = 995;
				} else if (!valid_email($from_email)) {
	...
	public function app_links() {
		if ($this->session->userdata("user_rank") < 1) {
			...
			$this->load->model('site_model');
			$settings = $this->site_model->get_website_settings()->result_array()[0];

			$data = array(
				"title" => "Admin - App Links",
				"settings" => $settings,
				"jscripts" => array(base_url("js/pages/admin_app_links.js"))
			);

			$this->load->view('admin/app_links', $data);
		}
	}

    function save_app_links() {
		$data = array();
		$data["error"] = 0;
		...
		if (DEMO_MODE == 1) {
			...
			if ($this->session->userdata("user_rank") < 1) {
				...
				$app_ios = $this->input->post('ios');
				$app_android = $this->input->post('android');

				if (!empty($app_ios) && filter_var($app_ios, FILTER_VALIDATE_URL) === false) {
					$data["error"] = 998;
					$data["error_msg"] = "Your iOS App URL is not valid.";
				} else if (!empty($app_android) && filter_var($app_android, FILTER_VALIDATE_URL) === false) {
					$data["error"] = 997;
					$data["error_msg"] = "Your Android App URL is not valid.";
				}
