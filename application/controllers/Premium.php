...
class Premium extends CI_Controller {
	...
	function activate_feature()
	{
		...
		if(!$this->session->userdata("user_id")) {	
	    	...
	    	if($feature != "see_who_loves_you" && $feature != "browse_invisibly" && $feature != "spot_light" && $feature != "featured_one_week" && $feature != "featured_one_month"&& $feature != "active_message") {
		    	$data["result"] = 998;
	    	} else {
				...
				if($feature_price > $user_coins) {
					...
					} else if($feature == "spot_light") {
						$this->user_model->activate_purchase($this->session->userdata("user_id"), "spot_light_coins");
						$this->user_model->update_info($this->session->userdata("user_id"), array("spot_light" => 1,'spot_light_date'=>date('Y-m-d h:i:sa')));	
					} else if($feature == "active_message") {
						$this->user_model->activate_purchase($this->session->userdata("user_id"), "active_message");
						$this->user_model->update_info($this->session->userdata("user_id"), array("active_message" => 1));
	...
	public function index()
	{
		if($this->session->userdata("user_firstform") != 1)
    	{
			...
			//echo '<pre/>',print_r($user_purchases); die();
			...
			foreach($user_purchases as $u_p)
			{
				...
				else if ($u_p["purchase_name"] == "active_message") {
				    $active_message=true;
				}else if($u_p["purchase_name"] == "spot_light_coins"){
