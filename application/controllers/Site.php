...
class Site extends CI_Controller {
		...
		public function index()
	{
		if (isset($_GET["installation"])) {
			setcookie('installation', $_GET["installation"]);
		}
		...
		if($this->session->userdata('user_id')) {
			...
			$this->load->model(['site_model', 'user_model']);
        ...
        $this->load->config('landing');
        $this->load->library('view');
		$this->view->load('welcome_header', $data);
        $this->view->load('welcome', $data);
        $this->view->load('welcome_footer', $data);
		}
	}
   ...
   function encounters(){
		...
		$users = $this->user_model->get_viewprofile_registered_users_page()->result_array();
		$users_count = $this->user_model->record_count();
		    	//echo '<pre>',print_r($users),'</pre>';exit();
	        ...
	        $data["jscripts"] = array(
		   	...
		   	$this->load->view('site/encounters', $data);
		   	
	   	
	}

   // this function redirect to the Landing page for login
    ...
    function add_encounters(){
    	...
    	if($this->session->userdata("user_firstform") != 1)
    	{
		    ...
		    $data["users"] = $this->site_model->get_enconter_users();
		    if(empty($data["users"])){
		    	redirect('home?action=no_encounters_found','refresh');
		    }
	        ...
	        $data["jscripts"] = array(
		   	...
		   	$this->load->view('site/likes', $data);
    ...
    function home() 
    {
    	...
    	if($this->session->userdata("user_firstform") != 1)
    	{
			...
			$spotlight_purchased=false;
			foreach($user_purchases as $u_p)
			{
				if($u_p["purchase_name"] == "spot_light_coins") 
					$spotlight_purchased = true;
			}
			$data["spotlight_purchased"] = $spotlight_purchased;
			$data['spotlight_users']=$this->user_model->get_spot_light_users();
