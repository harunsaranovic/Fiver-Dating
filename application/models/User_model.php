...
class user_model extends CI_Model
{
    ...
    public function get_spot_light_users()
    {
        $this->db->select('user.username AS username, user.is_fake AS is_fake, user_info.user_id AS uid,  user_info.birthday AS birthday,photo.thumb_url AS thumb_url');
        $this->db->from('user');
        $this->db->join('user_info', 'user_info.user_id = user.id');
        $this->db->join('photo', 'photo.id = user_info.main_photo', 'left');
        $this->db->where('user.status', 1);
        $this->db->where('user.first_step_form', 1);
        $this->db->where('user_info.spot_light', 1);
        $this->db->order_by('user_info.spot_light_date', 'desc');
        return $this->db->get()->result_array();
    }
    
    //Add Like 
    function add_like($from_user_id, $to_user_id)
    {
        $firstcheck = array(
        ...
        $this->db->insert('matchs_like', $newLike);
        if ($this->db->insert_id() > 0)
            return true;
        return false;
    }
    function add_unlike($from_user_id, $to_user_id)
    {
        $firstcheck = array(
            'from_user_id' => $from_user_id,
            'to_user_id' => $to_user_id
        );
        $first      = $this->db->get_where('matchs_like', $firstcheck)->row();
        $newLike    = array(
            'from_user_id' => $from_user_id,
            'to_user_id' => $to_user_id,
            'status' => 'Unlike'
    ...
    function get_like_registered_users_page($limit, $page, $user_id = 0)
    {
        ...
        $users        = $this->db->query("SELECT u.id AS uid, u.username, is_fake, p.url, p.thumb_url, p.status AS photostatus, ui.gender, ui.country, ui.city, ui.birthday, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL " . ONLINE_DELAY . " MINUTE, 1, 0) as is_online,  IF(ul.id IS NOT NULL, 1, 0) as st_love
                                   ...
                                   AND u.status = 1
                                   AND u.id NOT IN (SELECT to_user_id FROM matchs_like WHERE from_user_id='" . $current_user . "')
                                   AND u.id!='" . $current_user . "'
                                   AND u.id NOT IN (SELECT user_id FROM user_featured WHERE end_date >= DATE(UTC_TIMESTAMP()))
                                   ORDER BY u.register_date DESC LIMIT $page,$limit", array(
            $user_id
        ));
    ...
    //new function
    function get_viewprofile_registered_users_page()
    {
        //$current_user=$this->session->userdata('user_id');
        $users = $this->db->query("SELECT u.id AS uid, u.username, is_fake, p.url, p.thumb_url, p.status AS photostatus, ui.gender, ui.country, ui.city, ui.birthday, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL 0 MINUTE, 1, 0) as is_online FROM user u JOIN user_info ui ON ui.user_id = u.id LEFT JOIN photo p ON p.id = ui.main_photo WHERE first_step_form = 1 AND u.status = 1 AND u.id NOT IN (SELECT user_id FROM user_featured WHERE end_date >= DATE(UTC_TIMESTAMP())) ORDER BY RAND()  LIMIT 1");
        return $users;
    }
    
    function get_matches_registered_users_page($limit, $page, $user_id = 0)
    {
        $current_user = $this->session->userdata('user_id');
        $users        = $this->db->query("SELECT u.id AS uid, u.username, is_fake, p.url, p.thumb_url, p.status AS photostatus, ui.gender, ui.country, ui.city, ui.birthday, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL " . ONLINE_DELAY . " MINUTE, 1, 0) as is_online,  IF(ul.id IS NOT NULL, 1, 0) as st_love
                                   FROM user u 
                                   ...
                                   WHERE first_step_form = 1
                                   AND u.status = 1
                                   AND u.id IN (SELECT to_user_id FROM matchs_like WHERE status = 'like' AND from_user_id = '" . $current_user . "')
                                   AND u.id IN (SELECT from_user_id FROM matchs_like WHERE status = 'like' AND to_user_id = '" . $current_user . "')
                                   AND u.id NOT IN (SELECT user_id FROM user_featured WHERE end_date >= DATE(UTC_TIMESTAMP()))
                                   ORDER BY u.register_date DESC LIMIT $page,$limit", array(
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT fb_id FROM user WHERE fb_id = ?", array(
            $fb_user_id
        ));
    ...
    function create_social_account($social_id, $name, $email, $rank = 0)
    {
        $this->db->set(array(
            ...
            'rank' => $rank,
            'is_fake' => 0
        ))->set('register_date', 'NOW()', false)->set('last_activity_date', 'NOW()', false)->insert("user");
    ...
    function create($username, $password, $email, $status, $encrypt_id, $fb_id = "", $referer = "", $rank = 0, $is_fake = 0)
    {
        $this->db->set(array(
            ...
            'rank' => $rank,
            'is_fake' => $is_fake
        ))->set('register_date', 'UTC_TIMESTAMP()', false)->set('last_activity_date', 'UTC_TIMESTAMP()', false)->insert($this->table);
    ...
    function report($from_user_id, $profile_id, $type = 0)
    {
        $this->db->set(array(
            ...
            "type" => $type
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("report");
    ...
    {
        $query = $this->db->query("SELECT id FROM friend_notif WHERE user_id = ? AND seen = 0", array(
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_love WHERE to_user_id = ? AND viewed = 0", array(
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_profile_visit WHERE profile_id = ? AND viewed = 0", array(
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT from_user_id, to_user_id FROM user_friend WHERE ((from_user_id = ? AND to_user_id = ?) OR (from_user_id = ? AND to_user_id = ?)) AND status = 1", array(
            $user_id_1,
            $user_id_2,
            $user_id_2,
            $user_id_1
    ...
    function add_profile_visit($profile_id, $user_id)
    {
        $this->db->set(array(
            'profile_id' => $profile_id,
            'user_id' => $user_id
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_profile_visit");
    ...
    {
        $query = $this->db->query("SELECT id FROM user_profile_visit WHERE user_id = ? AND profile_id = ?", array(
            $user_id,
            $profile_id
        ));
    ...
    function send_request($profile_id, $user_id)
    {
        $this->db->set(array(
            ...
            'seen' => 0,
            'status' => 0
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_friend");
    ...
    {
        $query = $this->db->query("SELECT id FROM user_friend WHERE to_user_id = ? AND from_user_id = ? AND status = 0", array(
            $profile_id,
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_friend WHERE from_user_id = ? AND to_user_id = ? AND status = 0", array(
            $profile_id,
            $user_id
        ));
    ...
    {
        $this->db->delete('user_info', array(
            "user_id" => $user_id
        ));
    }
    ...
    {
        $query = $this->db->query("SELECT from_user_id, to_user_id FROM user_friend WHERE to_user_id = ? AND status = 0 AND seen = 0", array(
            $user_id
        ));
    ...
    function block($from_user_id, $profile_id)
    {
        ...
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_block");
        
        return $this->db->insert_id();
    }
    function checkAllowMessage($userid)
    {
        $this->db->select('active_message');
    ...
    {
        $this->db->delete('user_block', array(
            "from_user_id" => $from_user_id,
            "to_user_id" => $to_user_id
        ));
    }
    ...
    {
        $query = $this->db->query("SELECT id FROM user_block WHERE from_user_id = ? AND to_user_id = ?", array(
            $from_user_id,
            $to_user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_password_recovery WHERE encrypt_id = ? AND user_id = ?", array(
            $encrypt_id,
            $user_id
        ));
    ...
    {
        $this->db->delete('user_password_recovery', array(
            "encrypt_id" => $encrypt_id,
            "user_id" => $user_id
        ));
    }
    ...
    function add_password_recovery($encrypt_id, $user_id)
    {
        $this->db->set(array(
            'encrypt_id' => $encrypt_id,
            "user_id" => $user_id
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_password_recovery");
    ...
    {
        $query = $this->db->query("SELECT user_id, attr_name, attr_val FROM user_custom_field WHERE user_id = ? AND attr_name = ?", array(
            $user_id,
            $extra_field
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM " . $this->table . " WHERE username = ?", array(
            $username
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM " . $this->table . " WHERE username = ? AND id != ?", array(
            $username,
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM " . $this->table . " WHERE email = ? AND id != ?", array(
            $email,
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM " . $this->table . " WHERE email = ?", array(
            $email
        ));
    ...
    function get_profile_visits($user_id, $limit)
    {
        $visits = $this->db->query("SELECT DISTINCT(upv.user_id), u.id AS uid, u.username, is_fake, ui.gender, ui.country, ui.city, ui.birthday, p.thumb_url, p.status as photostatus, p.url, ui.gender, p.status AS photostatus, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL " . ONLINE_DELAY . " MINUTE, 1, 0) as is_online,  IF(ul.id IS NOT NULL, 1, 0) as st_love
                                    ...
                                    ORDER BY upv.date DESC LIMIT ?", array(
            $user_id,
            $user_id,
            $limit
        ));
    ...
    function get_lasts_users_who_loved($user_id, $limit)
    {
        $users = $this->db->query("SELECT ul.date as lovedate, u.id AS uid, u.username, is_fake, p.url, p.thumb_url, p.status AS photostatus, ui.gender, ui.country, ui.city, ui.birthday, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL " . ONLINE_DELAY . " MINUTE, 1, 0) as is_online, DATE_FORMAT(UTC_TIMESTAMP(), '%Y') - DATE_FORMAT(birthday, '%Y') - (DATE_FORMAT(UTC_TIMESTAMP(), '00-%m-%d') < DATE_FORMAT(birthday, '00-%m-%d')) AS age, IF(ul2.id IS NOT NULL, 1, 0) as st_love
                                   ...
                                   AND u.status = 1 
                                   AND ul.to_user_id = ?
                                   ORDER BY ul.date DESC LIMIT ?", array(
            $user_id,
            $user_id,
            $limit
        ));
    ...
    function add_friend_notif($user_id)
    {
        $this->db->set(array(
            'user_id' => $user_id,
            'seen' => 0
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("friend_notif");
    ...
    function activate_purchase($user_id, $purchase_name)
    {
        $this->db->set(array(
            ...
            'status' => 1
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_purchase");
    ...
    {
        $query = $this->db->query("SELECT id FROM user_purchase WHERE user_id = ? AND purchase_name = ? AND status = 1", array(
            $user_id,
            $purchase_name
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_friend WHERE to_user_id = ? AND status = 0", array(
            $user_id
        ));
    ...
    {
        $query = $this->db->query("SELECT id FROM user_friend WHERE (to_user_id = ? OR from_user_id = ?) AND status = 1", array(
            $user_id,
            $user_id
        ));
    ...
    function get_last_reported_users_page($limit, $page, $user_id = 0)
    {
        $users = $this->db->query("SELECT r.id AS rid, u.id AS uid, u.username, is_fake, p.url, p.thumb_url, p.status AS photostatus, ui.gender, ui.country, ui.city, ui.birthday, IF(u.last_activity_date >= UTC_TIMESTAMP() - INTERVAL " . ONLINE_DELAY . " MINUTE, 1, 0) as is_online,  IF(ul.id IS NOT NULL, 1, 0) as st_love
                                   ...
                                   AND r.status = 0
                                   AND r.type = 0
                                   ORDER BY r.date DESC LIMIT $page,$limit", array(
            $user_id
        ));
 ...
 AS cpt
    ...
    {
        $query = $this->db->query("SELECT id FROM report WHERE status = 0 AND type = ?", array(
            $type
        ));
    ...
    function add_love($from_user_id, $to_user_id)
    {
        $this->db->set(array(
            ...
            'to_user_id' => $to_user_id,
            'viewed' => 0
        ))->set('date', 'UTC_TIMESTAMP()', false)->insert("user_love");
    ...
    {
        $this->db->delete('user_friend', array(
            "from_user_id" => $user_id
        ));
        $this->db->delete('user_friend', array(
            "to_user_id" => $user_id
    ...
    {
        $this->db->delete('user_love', array(
            "from_user_id" => $user_id
        ));
        $this->db->delete('user_love', array(
            "to_user_id" => $user_id
    ...
    {
        $this->db->delete('user_profile_visit', array(
            "user_id" => $user_id
        ));
        $this->db->delete('user_profile_visit', array(
            "profile_id" => $user_id
